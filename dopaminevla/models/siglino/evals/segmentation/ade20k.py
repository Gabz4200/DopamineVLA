...
def parse_args():
    ...
    p.add_argument(
        "--root_dir",
        type=str,
        default="/lustre1/tier2/users/sofian.chaybouti/ade20k/ADEChallengeData2016",
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to write JSON metrics; defaults to derived training output dir",
    )
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print(
            f"Epoch {epoch + 1}/{args.epochs} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | {metric_name}: {metric:.4f} | Best({metric_name}): {best_metric:.4f} @ epoch {best_epoch}"
        )
