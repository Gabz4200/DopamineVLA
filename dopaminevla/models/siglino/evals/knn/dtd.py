...
def main():
    ...
    parser.add_argument(
        "--train_list", type=str, required=True, help="Train split file (e.g. train1.txt)"
    )
    parser.add_argument(
        "--val_list", type=str, required=True, help="Val/Test split file (e.g. test1.txt)"
    )
    ...
    if (not using_distributed) or (rank == 0):
        ...
        with open(os.path.join(args.output_dir, "dtd_knn.json"), "w") as f:
