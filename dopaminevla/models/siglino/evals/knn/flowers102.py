...
@torch.no_grad()
def main():
    parser = argparse.ArgumentParser("Flowers-102 kNN Classification")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to training images (one folder per class)",
    )
    parser.add_argument(
        "--val_images_root",
        type=str,
        required=True,
        help="Path to validation/test images (one folder per class)",
    )
    ...
    # Datasets
    train_dataset = Flowers102Dataset(args.train_images_root)
    val_dataset = Flowers102Dataset(args.val_images_root)
    ...
    if using_distributed:
        totals = torch.tensor(
            [num_correct_dino, num_correct_siglip, num_correct_ens, num_samples],
            device=device,
            dtype=torch.long,
        )
    ...
    if (not using_distributed) or (rank == 0):
        ...
        with open(os.path.join(args.output_dir, "flowers102_knn.json"), "w") as f:
