...
def main():
    ...
    # Data paths
    parser.add_argument(
        "--images_path", type=str, required=True, help="Path to the ImageNet validation images"
    )
    parser.add_argument(
        "--train_images_path", type=str, required=True, help="Path to the ImageNet training images"
    )
    parser.add_argument(
        "--imagenet_mappings",
        type=str,
        required=True,
        help="Path to the ImageNet mappings JSON file",
    )
    ...
    if (not using_distributed) or (rank == 0):
        ...
        with open(os.path.join(args.output_dir, "imagenet_knn.json"), "w") as f:
