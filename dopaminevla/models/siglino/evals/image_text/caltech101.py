...
class Caltech101Dataset(Dataset):
    """Loads Caltech-101 images from folder structure: one folder per class."""
    ...
    def __init__(self, root_dir):
        ...
        # Classes: subdirectories directly under root
        ...
        for c in class_names:
            ...
            for p in sorted(cdir.rglob("*")):
                if p.is_file() and p.suffix.lower() in exts:
                    images.append((str(p), c, self.cat_to_id[c]))

        self.samples = images
        print(f"Found {len(self.samples)} images across {len(class_names)} classes in {self.root}")
...
def setup_text_embeddings_for_caltech(
    class_names,
    dinotxt_model,
    dinotxt_tokenizer,
    siglip2_model_name,
    ...
    # Convert class folder names into display prompts (lower-case, underscores to spaces)
...
@torch.no_grad()
def main():
    parser = argparse.ArgumentParser("Caltech-101 Zero-shot Classification (DINOv3/SigLIP2 multi-teacher) with ensembling")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Caltech-101 root directory containing class folders",
    )
    ...
    # Dataset / loader
    dataset = Caltech101Dataset(args.data_root)
    ...
    # Text embeddings
    text_embeds_dinotxt, text_embeds_siglip2, id_to_cat = setup_text_embeddings_for_caltech(
        class_names,
        dinotxt,
        dinotxt_tokenizer,
    ...
    if rank == 0:
        print("Starting evaluation on Caltech-101...")
    ...
    if (not using_distributed) or (rank == 0):
        ...
        print(f"DINOv3/dinotxt Caltech-101 Accuracy: {accuracy_dinotxt:.4f} ({num_correct_dinotxt}/{num_samples})")
        print(f"SigLIP2 Caltech-101 Accuracy: {accuracy_siglip2:.4f} ({num_correct_siglip2}/{num_samples})")
        print(f"Ensemble (entropy-weighted) Caltech-101 Accuracy: {accuracy_ensemble:.4f} ({num_correct_ensemble}/{num_samples})")
        ...
        with open(os.path.join(args.output_dir, "caltech101.json"), "w") as f:
