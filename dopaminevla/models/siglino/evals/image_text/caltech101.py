...
def main():
    parser = argparse.ArgumentParser(
        "Caltech-101 Zero-shot Classification (DINOv3/SigLIP2 multi-teacher) with ensembling"
    )
    ...
    if (not using_distributed) or (rank == 0):
        ...
        print(
            f"DINOv3/dinotxt Caltech-101 Accuracy: {accuracy_dinotxt:.4f} ({num_correct_dinotxt}/{num_samples})"
        )
        print(
            f"SigLIP2 Caltech-101 Accuracy: {accuracy_siglip2:.4f} ({num_correct_siglip2}/{num_samples})"
        )
        print(
            f"Ensemble (entropy-weighted) Caltech-101 Accuracy: {accuracy_ensemble:.4f} ({num_correct_ensemble}/{num_samples})"
        )
