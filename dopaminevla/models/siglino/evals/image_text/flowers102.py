...
class Flowers102Dataset(Dataset):
    """Oxford Flowers-102 validation split (zero-shot) with numeric class folders and a cat_to_name mapping."""

    def __init__(self, images_root: str, cat_to_name_json: str):
        self.images_root = Path(images_root)
        if not self.images_root.exists():
            raise FileNotFoundError(f"Images root not found: {self.images_root}")

        with open(cat_to_name_json, "r") as f:
            raw_map = json.load(f)  # keys like "1".."102"
        # Sort classes numerically by their folder name and restrict to those present in mapping
        class_dirs = [p for p in self.images_root.iterdir() if p.is_dir()]
        class_keys = []
        for p in class_dirs:
            name = p.name
            if name in raw_map:  # keep only classes present in mapping
...
def setup_text_embeddings_for_flowers102(
    id_to_cat,  # idx -> folder key (e.g., "21")
    cat_to_name,  # folder key -> display name (string)
    dinotxt_model,
    dinotxt_tokenizer,
    siglip2_model_name,
    device,
    ...
    ordered_keys = [id_to_cat[i] for i in range(len(id_to_cat))]
    display_names = [cat_to_name[k] for k in ordered_keys]
    id_to_cat_out = {i: k for i, k in enumerate(ordered_keys)}  # idx -> folder key
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Flowers-102 validation directory containing numeric class folders",
    )
    parser.add_argument(
        "--cat_to_name",
        type=str,
        required=True,
        help="JSON mapping class number (string) -> class name",
    )
    ...
    # Dataset / loader
    dataset = Flowers102Dataset(args.images_root, args.cat_to_name)
    # Prepare display-name mapping for text embeddings
    with open(args.cat_to_name, "r") as f:
        raw_map = json.load(f)
    # Ensure lowercase for prompts
    cat_to_name_lower = {k: v.lower() for k, v in raw_map.items()}
    ...
    # Text embeddings
    text_embeds_dinotxt, text_embeds_siglip2, id_to_cat = setup_text_embeddings_for_flowers102(
        dataset.id_to_cat,
        cat_to_name_lower,
        dinotxt,
        dinotxt_tokenizer,
    ...
    if rank == 0:
        print("Starting evaluation on Flowers-102 (valid)...")
    for batch in tqdm(dataloader, desc="Processing batches", disable=(rank != 0)):
        ...
        for pred_id_dino, pred_id_siglip, pred_id_ens, gt_cat in zip(
            ...
            cat_dino = id_to_cat[pred_id_dino.item()]  # folder key like "21"
    ...
    if (not using_distributed) or (rank == 0):
        ...
        with open(os.path.join(args.output_dir, "flowers102.json"), "w") as f:
