...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4096,
        help="Batch size for encoding text prompts (templates x classes).",
    )
    parser.add_argument(
        "--no_openai_templates",
        action="store_true",
        help="Disable OpenAI template averaging; use a single simple prompt per class.",
    )
    ...
    if (not using_distributed) or (rank == 0):
        ...
        with open(os.path.join(args.output_dir, "imagenet.json"), "w") as f:
