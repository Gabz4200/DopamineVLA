...
class DTDDataset(Dataset):
    """DTD dataset loader: one folder per class under images_root."""
...
def setup_text_embeddings_for_dtd(
    class_names,
    dinotxt_model,
    dinotxt_tokenizer,
    siglip2_model_name,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="DTD images directory (one folder per class)",
    )
    ...
    # Dataset / loader
    dataset = DTDDataset(args.images_root)
    ...
    # Text embeddings
    text_embeds_dinotxt, text_embeds_siglip2, id_to_cat = setup_text_embeddings_for_dtd(
        class_names,
        dinotxt,
        dinotxt_tokenizer,
    ...
    if rank == 0:
        print("Starting evaluation on DTD...")
    for batch in tqdm(dataloader, desc="Processing batches", disable=(rank != 0)):
        ...
        # Entropy-weighted ensemble of logits (beta=5.0)
        T_dino = 1.0
        T_siglip = 1.0
    ...
    if using_distributed:
        totals = torch.tensor(
            [num_correct_dinotxt, num_correct_siglip2, num_correct_ens, num_samples],
            device=args.device,
            dtype=torch.long,
        )
        dist.all_reduce(totals, op=dist.ReduceOp.SUM)
        num_correct_dinotxt, num_correct_siglip2, num_correct_ens, num_samples = totals.tolist()
    ...
    if (not using_distributed) or (rank == 0):
        ...
        out = {
            ...
            "num_correct_ensemble": num_correct_ens,
            "num_samples": num_samples,
            "accuracy_dinotxt": accuracy_dinotxt,
            "accuracy_siglip2": accuracy_siglip2,
            "accuracy_ensemble": accuracy_ens,
            "accuracy": accuracy_dinotxt,
            "accuracy_per_adaptor": {
                "dinotxt": accuracy_dinotxt,
                "siglip2": accuracy_siglip2,
            },
            "accuracy_per_adaptor_percent": {
                "dinotxt": accuracy_dinotxt * 100.0,
                "siglip2": accuracy_siglip2 * 100.0,
            },
            "accuracy_dinotxt_percent": accuracy_dinotxt * 100.0,
            "accuracy_siglip2_percent": accuracy_siglip2 * 100.0,
            "accuracy_ensemble_percent": accuracy_ens * 100.0,
        }
        with open(os.path.join(args.output_dir, "dtd.json"), "w") as f:
