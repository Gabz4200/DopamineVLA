...
def main():
    parser = argparse.ArgumentParser(
        "FGVC-Aircraft (Family) Zero-shot Classification (DINOv3/SigLIP2 multi-teacher) with ensembling"
    )
    ...
    for batch in tqdm(dataloader, desc="Processing batches", disable=(rank != 0)):
        ...
        probs_ens = combine_logits(
            logits_per_text_dino, logits_per_text_siglip, mode="entropy_weighted", beta=5.0
        )
    ...
    if (not using_distributed) or (rank == 0):
        ...
        print(
            f"DINOv3/dinotxt FGVC-Aircraft (family) Accuracy: {accuracy_dinotxt:.4f} ({num_correct_dinotxt}/{num_samples})"
        )
        print(
            f"SigLIP2 FGVC-Aircraft (family) Accuracy: {accuracy_siglip2:.4f} ({num_correct_siglip2}/{num_samples})"
        )
        print(
            f"Ensemble (entropy-weighted) FGVC-Aircraft (family) Accuracy: {accuracy_ensemble:.4f} ({num_correct_ensemble}/{num_samples})"
        )
        ...
        with open(os.path.join(args.output_dir, "aircraft.json"), "w") as f:
