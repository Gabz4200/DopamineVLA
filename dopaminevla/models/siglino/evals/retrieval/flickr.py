...
def main():
    parser = argparse.ArgumentParser(description="Flickr30k retrieval with multi-teacher DistilTransformer (DINOv3 + SigLIP2)")
    ...
    parser.add_argument(
        ...
        help="Real DINOv3 model name for torch.hub",
    )
    parser.add_argument(
        "--siglip2_model_name",
        type=str,
        default="google/siglip2-so400m-patch16-naflex",
        help="SigLIP2 model name for text encoder",
    )
    ...
    test_data = dataset["test"].filter(lambda x: x.get("split", "test") == "test")
    ...
    # Note: Flickr30k HF dataset has 'image' (PIL) and 'caption' (list of strings)
    for img_idx, item in enumerate(tqdm(test_data, desc="Preparing data")):
        img = item["image"]
        if not isinstance(img, Image.Image):
            # Try to load if it's a path
    ...
    print("=" * 60)
    t2i_dino, i2t_dino = compute_retrieval_metrics_from_similarity(sim_dino, image_to_texts_map)
    ...
    print("=" * 60)
    t2i_ens, i2t_ens = compute_retrieval_metrics_from_similarity(logits_ens, image_to_texts_map)

    print("\n" + "=" * 60)
    print("FLICKR30K EVALUATION RESULTS")
    print("=" * 60)
    ...
    results = {
        ...
        "dinotxt": {
            "standard": {
                "text_to_image_retrieval": {
                    "recall_at_1": t2i_dino[1],
                    "recall_at_5": t2i_dino[5],
                    "recall_at_10": t2i_dino[10],
                },
                "image_to_text_retrieval": {
                    "recall_at_1": i2t_dino[1],
                    "recall_at_5": i2t_dino[5],
                    "recall_at_10": i2t_dino[10],
                },
            },
        },
        "siglip2": {
            "standard": {
                "text_to_image_retrieval": {
                    "recall_at_1": t2i_siglip[1],
                    "recall_at_5": t2i_siglip[5],
                    "recall_at_10": t2i_siglip[10],
                },
                "image_to_text_retrieval": {
                    "recall_at_1": i2t_siglip[1],
                    "recall_at_5": i2t_siglip[5],
                    "recall_at_10": i2t_siglip[10],
                },
            },
        },
        "ensemble_entropy_weighted": {
            "standard": {
                "text_to_image_retrieval": {
                    "recall_at_1": t2i_ens[1],
                    "recall_at_5": t2i_ens[5],
                    "recall_at_10": t2i_ens[10],
                },
                "image_to_text_retrieval": {
                    "recall_at_1": i2t_ens[1],
                    "recall_at_5": i2t_ens[5],
                    "recall_at_10": i2t_ens[10],
                },
            },
        },
    }

    json_filename = "flickr30k.json"
