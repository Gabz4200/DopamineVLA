...
class SigLinoHFModel(SigLinoPreTrainedModel):
    ...
    def _get_onnx_wrapper(self) -> torch.nn.Module:
        ...
        The wrapper handles spatial_shapes computation from pixel_values
        and flattens the dict output to a tuple.
        """
        core = self.model
    ...
    def export_to_onnx(cls, model_path: str, output_path: str, **onnx_export_kwargs: Any) -> None:
        ...
        Args:
            model_path: Path to the pretrained model or HuggingFace Hub ID
            output_path: Path where the ONNX model will be saved
            **onnx_export_kwargs: Additional arguments passed to torch.onnx.export
        """
        model = cls.from_pretrained(model_path, torch_dtype=torch.float32, device_map="cpu")
        model.eval()
        ...
        with torch.no_grad():
            torch.onnx.export(
                ...
                output_names=[
                    "patch_features_dinov3",
                    "patch_features_siglip",
                    "patch_features_siglino",
                    "summary_features_dinov3",
                    "summary_features_siglip",
                    "summary_features_siglino",
                ],
                dynamic_axes={
                    "pixel_values": {0: "batch_size"},
                    "patch_features_dinov3": {0: "batch_size"},
                    "patch_features_siglip": {0: "batch_size"},
                    "patch_features_siglino": {0: "batch_size"},
                    "summary_features_dinov3": {0: "batch_size"},
                    "summary_features_siglip": {0: "batch_size"},
                    "summary_features_siglino": {0: "batch_size"},
                },
            )
